...
phyluce_probe_run_multiple_lastzs_sqlite \
    --db results/tetrapods/alignment_tetrapods.sqlite \
    --output results/tetrapods/alignment-tetrapods-lastz \
    ...
    --genome-base-path ./ \
    --probefile /home/lucie/PhyloMut/UCE/UCE_probes/Tetrapods-UCE-5Kv1-DUPE-SCREENED.fasta \
    --cores 12
    
# Extract the region in the ref with a flanking sequence of 1000 bp each side
phyluce_probe_slice_sequence_from_genomes \
    --lastz results/tetrapods/alignment-tetrapods-lastz \
    --conf genomes.conf \
    --flank 1000 \
    --name-pattern "Tetrapods-UCE-5Kv1-DUPE-SCREENED.fasta_v_{}.lastz.clean" \
    --output results/tetrapods/alignment-tetra-fasta

# Match back the contigs (fasta) to probes
phyluce_assembly_match_contigs_to_probes \
    --contigs results/tetrapods/alignment-tetra-fasta \
    --probes /home/lucie/PhyloMut/UCE/UCE_probes/Tetrapods-UCE-5Kv1-DUPE-SCREENED.fasta \
    --output results/tetrapods/uce-search-results
mkdir -p results/tetrapods/taxon-sets/all

## Create the data matrix to configurate the files
phyluce_assembly_get_match_counts \
    --locus-db results/tetrapods/uce-search-results/probe.matches.sqlite \
    --taxon-list-config taxon-set.conf \
    --taxon-group 'all' \
    --incomplete-matrix \
    --output results/tetrapods/taxon-sets/all/all-taxa-incomplete.conf
mkdir results/tetrapods/taxon-sets/all/log

# Get all the probes for all taxa in a fasta format all-taxa-incomplete.fasta
phyluce_assembly_get_fastas_from_match_counts \
    --contigs results/tetrapods/alignment-tetra-fasta \
    --locus-db results/tetrapods/uce-search-results/probe.matches.sqlite \
    --match-count-output results/tetrapods/taxon-sets/all/all-taxa-incomplete.conf \
    --output results/tetrapods/taxon-sets/all/all-taxa-incomplete.fasta \
    --incomplete-matrix results/tetrapods/taxon-sets/all/all-taxa-incomplete.incomplete \
    --log-path results/tetrapods/taxon-sets/all/log

# With no trimming and mafft aligner, output one fasta for each probes
phyluce_align_seqcap_align \
    --fasta results/tetrapods/taxon-sets/all/all-taxa-incomplete.fasta \
    --output results/tetrapods/taxon-sets/all/mafft-nexus-internal-trimmed \
    --taxa 74 \
    --aligner mafft \
    --cores 6 \
    --incomplete-matrix \
...
phyluce_assembly_explode_get_fastas_file \
    --input results/tetrapods/taxon-sets/all/all-taxa-incomplete.fasta \
    --output results/tetrapods/exploded-fastas \
    --by-taxon

# Clean the name of sequences
phyluce_align_remove_locus_name_from_nexus_lines \
    --alignments results/tetrapods/taxon-sets/all/mafft-nexus-internal-trimmed \
    --output results/tetrapods/taxon-sets/all/mafft-nexus-internal-trimmed-clean \
    --input-format fasta
    --cores 6 \
    --log-path results/tetrapods/taxon-sets/all/log
...
phyluce_align_get_only_loci_with_min_taxa \
    ...
    --taxa 74 \
    --percent 0.75 \
    --output results/tetrapods/taxon-sets/all/mafft-nexus-internal-trimmed-clean-75p \
    --cores 6 \
    --log-path results/tetrapods/taxon-sets/all/log
